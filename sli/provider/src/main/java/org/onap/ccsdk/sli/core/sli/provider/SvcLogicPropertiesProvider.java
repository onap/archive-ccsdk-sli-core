...
public class SvcLogicPropertiesProvider {
    ...
    public SvcLogicPropertiesProvider() {
        sliPropertiesFileResolvers.add(new SdncConfigEnvVarFileResolver(
                "Using property file (1) from environment variable"
        ));
        sliPropertiesFileResolvers.add(new CoreDefaultFileResolver(
                "Using property file (2) from default directory"
        ));

        sliPropertiesFileResolvers.add(new JREFileResolver(
                "Using property file (3) from JRE argument", SvcLogicPropertiesProvider.class
        ));
        sliPropertiesFileResolvers.add(new KarafRootFileResolver(
        ...
        if (propertiesFile != null) {
            try(FileInputStream fileInputStream = new FileInputStream(propertiesFile)) {
                ...
                LOG.error("Failed to load properties for file: {}", propertiesFile.toString(),
                        new ConfigurationException("Failed to load properties for file: "
        ...
        if (properties == null) {
            reportFailure("Missing configuration properties resource(3)",
                    new ConfigurationException("Missing configuration properties resource(3): "
