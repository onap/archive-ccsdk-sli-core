...
/**
 * Base class responsible for parsing business logic for database configuration
 * from given <code>Properties</code>.
 */
public abstract class BaseDBConfiguration {
	...
	/**
	 * Property key with a properties configuration File for db database
	 * connection name
	 */
	 ...
	 * parsed
	 */
	private static final String DEFAULT_CONNECTION_TIMEOUT = "-1";

	/**
	 * default value when the request timeout is not present or cannot be parsed
	 */
	private static final String DEFAULT_REQUEST_TIMEOUT = "-1";
	 ...
	 *         <code>DEFAULT_CONNECTION_TIMEOUT</code> if not present
	 */
	public int getConnTimeout() {
		try {
			String value = properties.getProperty(CONNECTION_TIMEOUT, DEFAULT_CONNECTION_TIMEOUT);
	...
	public int getRequestTimeout() {
		try {
			String value = properties.getProperty(REQUEST_TIMEOUT, DEFAULT_REQUEST_TIMEOUT);
			return Integer.parseInt(value);
		} catch (Exception exc) {
			return Integer.parseInt(DEFAULT_REQUEST_TIMEOUT);
		}
	}
